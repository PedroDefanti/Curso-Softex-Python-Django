...
class UserService:
    ...
    def register_user(
        ...
        Caso os campos atendas as requisições, faça o hash da senha e salve usando o método create_user da User Model
        """
        if len(senha)<8:
            
            return False,'senha INVÁLIDA'
        
        if len(email)<10 and '@'not in email and email[-4:]!='.com':
            return False,'email INVÁLIDO'
        
        if nome_completo.isalpha() and nome_completo=='':
            return False,'nome INVÁLIDA'
        senha_hash=hash_senha(senha)
        self.user_model=UserModel()
        self.user_model.create_user(senha_hash,email,nome_completo,perfil)
    ...
    def login_user(self, email: str, senha: str) -> tuple[dict | None, str]:
        ...
        """
        if email !='' and senha !='':
             user_model=self.user_model.find_user_by_email(email)
             if verificar_senha(senha,user_model['senha_hash']):
                 return self._safe_user_data(user_model)
        return None,'Email e/ou senha INVÁLIDA'
